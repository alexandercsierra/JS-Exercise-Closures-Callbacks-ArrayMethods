...
function processDuplicateFree(arr, callback) {
  return callback(arr.filter((item, index) => arr.indexOf(item) == index));
}
...
function firstNamesAllCaps(runners) {
  var upper = runners.map(function(person){
...
function tallyUpDonations(runners) {
  const stuff = runners.reduce((total, num) => { return total += num.donation}, 0);
  return stuff;
}
